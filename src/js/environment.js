...
function buildEnvironment() {
    const {W, H, env} = World;
    const f = World.field;
    const sHum = randRange(World.rng, 120, 420), sLig = randRange(World.rng, 200, 700),
        sNut = randRange(World.rng, 150, 600), sWat = randRange(World.rng, 180, 520);
        ...
        for (let x = 0; x < W; x++) {
            ...
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    if (dx === 0 && dy === 0) continue;
                    const nx = x + dx, ny = y + dy;
                    if (nx >= 0 && ny >= 0 && nx < W && ny < H) cnt += water[ny * W + nx];
                }
            }
            cleaned[i] = cnt >= 1 ? 1 : 0;
        }
    ...
    for (let i = 0; i < W * H; i++) {
        if (env.water[i]) {
            env.humidity[i] = clamp(env.humidity[i] * 0.88 + 0.12 * 1, 0, 1);
            env.nutrient[i] = clamp(env.nutrient[i] + 0.04, 0, 1);
        }
    }
}
