...
function draw() {
    ...
    const t = World.tiles;
    const b = World.biomass;
    ctx.imageSmoothingEnabled = false;
    ctx.fillStyle = '#050812';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    const ovH = document.getElementById('ovHumidity').checked;
    const ovL = document.getElementById('ovLight').checked;
    const ovN = document.getElementById('ovNutrient').checked;
    const ovW = document.getElementById('ovWater').checked;
    const ovT = document.getElementById('ovTrail').checked;
    if (ovH || ovL || ovN || ovW) {
    ...
    if (ovT) {
        ...
        const img = ctx.createImageData(W, H);
        let max = 0;
        for (let i = 0; i < Slime.trail.length; i++) max = Math.max(max, Slime.trail[i]);
        ...
        for (let y = 0; y < H; y++) {
            for (let x = 0; x < W; x++) {
                ...
                img.data[k + 1] = g;
                img.data[k + 2] = b;
                img.data[k + 3] = Math.floor(255 * Math.min(1, 0.15 + 0.85 * p));
            }
        }
        const off = document.createElement('canvas');
        off.width = W;
